 ...
 */
const getPinsAsync = co.wrap(function*(url, total) {
  let pins = [];
  if (url.slice(-1) !== '/') url += '/';
  ...
  while (i <= total) {
    ...
    const html = yield util.getHtmlAsync(url, query);
...
HuabanBoard.prototype.initAsync = co.wrap(function*() {
  const html = yield util.getHtmlAsync(this.url);
  const board = getBoard(html);
  ...
  const pins = yield getPinsAsync(this.url, board['pin_count']);
  return this.pins = pins;
});

HuabanBoard.prototype.downloadBoardAsync = co.wrap(function*(concurrency) {
  ...
  return yield Promise.map(this.pins, function(pin, index, length) {
    ...
    return util.downloadAsync(pin.src, dest)
      .then(function() {
